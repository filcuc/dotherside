...
QByteArray createSignature(const T &functionDefinition)
{
    ...
    for (size_t i = 0; i < parameters.size(); ++i) {
        if (i != 0)
            arguments += ",";
        arguments += QMetaType(parameters[i].metaType).name();
    }
                                              ...
                                              const PropertyDefinitions &propertyDefinitions)
{
    ...
    for (const SignalDefinition &signal : signalDefinitions) {
        ...
        signalBuilder.setReturnType(QMetaType(QMetaType::Void).name());
    ...
    for (const SlotDefinition &slot : slotDefinitions) {
        ...
        methodBuilder.setReturnType(QMetaType(slot.returnType).name());
    ...
    for (const PropertyDefinition &property : propertyDefinitions) {
        ...
        const QByteArray typeName = QMetaObject::normalizedType(QMetaType(property.type).name());
